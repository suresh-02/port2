            ...
            <ul className="hidden list-none flex-row gap-10 sm:flex">
              {navLinks.map((link, index) => (
                <a
                  ...
                  } hover:text-slate-00 relative cursor-pointer text-[18px] font-medium transition-all duration-300 dark:hover:text-white`}
                  onClick={() => {
                    setActive(link.href);
                  }}
                >
    ...
    >
      {title.split("").map((letter, index) =>
        letter === " " ? (
          ...
          <motion.span
            variants={animation}
            className="whitespace relative inline-block"
            key={index}
          >
            {letter}
          </motion.span>
